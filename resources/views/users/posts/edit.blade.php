     ...
     <form action="{{ route('post.update', $post->id) }}" method="post" enctype="multipart/form-data">
            ...
            @foreach ($all_categories as $category)
                <div class="form-check form-check-inline">
                    @if (in_array($category->id, $selected_categories))
                        <input type="checkbox" name="category[]" id="{{ $category->name }}" value="{{ $category->id }}" class="form-check-label" checked>
                    @else
        ...
        <div class="row mb-4">
            <div class="col-6">
                ...
                <input type="file" name="image" id="image" class="mt-1 form-control" aria-describedby="image-info">
